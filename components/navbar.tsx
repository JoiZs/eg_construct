...
const NavBar:React.FC = (props:Props) =>{
    ...
    useEffect(()=>{
        ...
        if(section[0]===1 ){
            gsap.fromTo('#homeDiv',{ translateY:'50%', opacity:0},{duration:0.5 ,translateY:'0', opacity:1, ease:'power2.out'})
            gsap.fromTo('#homeTitle',{translateX:'-100%', opacity:0},{duration:0.8 ,translateX:'0%', opacity:1, delay:0.2})
            gsap.fromTo('#homeSubt',{translateX:'-100%', opacity:0},{duration:0.8 ,translateX:'0%', opacity:1, delay:0.6, ease:'expo.out'})
            gsap.fromTo('#servDivDetail',{ opacity:1},{duration:0 , opacity:0, ease:'power2.inOut'})
            gsap.fromTo('#servList',{ opacity:1},{duration:0 , opacity:0, ease:'power2.inOut'})
            
        }else if(section[1] === 1){
            gsap.fromTo('#servList',{translateY:'5%',  opacity:0},{duration:0.5 ,translateY:'0%', opacity:1, ease:'power1.out'})
            gsap.fromTo('#servTrust',{translateY:'5%', opacity:0},{duration:0.5 ,translateY:'0%', opacity:1, ease:'power1.out'})
            gsap.fromTo('#servDivDetail',{ translateY:'-5rem', opacity:0},{duration:2 ,translateY:'0', opacity:1, ease:'power2.out'})
            ...
            gsap.fromTo('#homeDiv',{ translateY:'0', opacity:1},{duration:0.1 ,translateY:'50%', opacity:0, ease:'power2.inOut'})
    ...
    return (
        ...
        <div className='h-screen fixed right-0 hidden md:flex justify-between flex-col dark:text-white w-12 z-20'>
            <div className='py-10 cursor-pointer'><div className='absolute w-32 right-6'><Link href='/' passHref><Image id='logoT' src={selectLogo[darkModeOn]} alt='logo' /></Link></div></div>
